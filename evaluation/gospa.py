...
def hausdorff_distance(x, y):
    ...
    num_element_x = len(x)
    num_element_y = len(y)
    all_distance = np.zeros([num_element_x, num_element_y])
    for idx_x in range(num_element_x):
        for idx_y in range(num_element_y):
            all_distance[idx_x, idx_y] = np.linalg.norm(x[idx_x] - y[idx_y])
    h_distance = np.max([np.max(np.min(all_distance, axis=0)), np.max(np.min(all_distance, axis=1))])

    return h_distance

def rotated_iou(x, y):
    """ Rotated IOU metric function. """
    box1 = np.hstack([x[0:4], x[4]/np.pi*180]).reshape(1,5)
    box2 = np.hstack([y[0:4], y[4]/np.pi*180]).reshape(1,5)
    iou = pb.rotated_iou_distance(box1, box2)
    return iou[0,0]
