...
class Evaluator:
    ...
    def evaluate(self, dataset_list, metrics_list):
                ...
                try:
                    ...
                    if config['USE_PARALLEL']:
                        ...
                        for curr_seq in sorted(seq_list):
                            res[curr_seq], data[curr_seq] = eval_sequence(curr_seq, dataset, tracker, class_list, metrics_list,
                                                          metric_names, return_data=True)
                    ...
                    output_data[dataset_name][tracker] = data
        ...
        return output_res, output_msg, output_data


@_timing.time
def eval_sequence(seq, dataset, tracker, class_list, metrics_list, metric_names, return_data=False):
    ...
    for cls in class_list:
        seq_res[cls] = {}
        data[cls] = dataset.get_preprocessed_seq_data(raw_data, cls)
        for metric, met_name in zip(metrics_list, metric_names):
            seq_res[cls][met_name] = metric.eval_sequence(data[cls])
    if not return_data:
        return seq_res
    else:
        return seq_res, data
