...
class Kitti2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        self.prematch_result = self.config['PREMATCH_RESULT']
        self.prematch_distance_threshold = self.config['PREMATCH_THRESHOLD']

        # Get classes to eval
        self.valid_classes = ['car', 'pedestrian', 'cyclist', 'truck']
        self.class_list = [cls.lower() if cls.lower() in self.valid_classes else None
                           for cls in self.config['CLASSES_TO_EVAL']]
        if not all(self.class_list):
            raise TrackEvalException('Attempted to evaluate an invalid class. Only classes [car, pedestrian, cyclist] are valid.')
        # KITTI
        ...
        # VOD
        self.class_name_to_class_id = {'car': 1, 'pedestrian': 2, 'cyclist': 3, 'rider': 4, 'bicycle': 5, 'bicycle_rack': 6,
                                    'human_depiction': 7, 'moped_scooter': 8, 'motor': 9, 'truck': 10,
                                    'ride_other': 11, 'vehicle_other': 12, 'ride_uncertain': 13, 'dontcare': 14, 'other': 15}
        # TJ4D
        # self.class_name_to_class_id = {'car': 1, 'pedestrian': 2, 'cyclist': 3, 'truck': 4, 'other': 5}
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                ...
                # # Calculate the 4 corners (in counter-clockwise order) for the 2D bbox in BEV plane
                # corners = np.zeros([time_data.shape[0], 8])
                # for obj_idx in range(time_data.shape[0]):
                #     rotation = -(time_data[obj_idx][-2] + np.pi/2)
                #     rot_mat = np.array([[np.cos(rotation),-np.sin(rotation),0],
                #                         [np.sin(rotation), np.cos(rotation),0],
                #                         [0,0,1]])
                #     corners_at_origin = np.array([  [-time_data[obj_idx][11]/2, time_data[obj_idx][11]/2,time_data[obj_idx][11]/2,-time_data[obj_idx][11]/2],
                #                                     [-time_data[obj_idx][12]/2,-time_data[obj_idx][12]/2,time_data[obj_idx][12]/2, time_data[obj_idx][12]/2],
                #                                     [1,1,1,1]])
                #     corners_for_this_object = rot_mat @ corners_at_origin + time_data[obj_idx][13:16].reshape(-1,1)
                #     corners[obj_idx] = corners_for_this_object[0:-1, :].reshape([8,], order='F') # In x0y0x1y1x2y2x3y3 format
                # raw_data['dets'][t] = np.atleast_2d(corners)

                # Use detection center point to calculate similarity
                # raw_data['dets'][t] = np.atleast_2d(time_data[:, 13:15])  # xy
                raw_data['dets'][t] = np.atleast_2d(time_data[:, 10:16])    # hwlxyz
                ...
                raw_data['dets'][t] = np.empty((0, 6))  # hwlxyz
            ...
            if is_gt:
                if time_key in ignore_data.keys():
                    ...
                    # # Calculate the 4 corners (in counter-clockwise order) for the 2D bbox in BEV plane
                    # corners = np.zeros([time_ignore.shape[0], 8])
                    # for obj_idx in range(time_ignore.shape[0]):
                    #     rot_mat = np.array([[np.cos(time_ignore[obj_idx][-2]),-np.sin(time_ignore[obj_idx][-2]),0],
                    #                         [np.sin(time_ignore[obj_idx][-2]), np.cos(time_ignore[obj_idx][-2]),0],
                    #                         [0,0,1]])
                    #     corners_at_origin = np.array([  [-time_ignore[obj_idx][11]/2, time_ignore[obj_idx][11]/2,time_ignore[obj_idx][11]/2,-time_ignore[obj_idx][11]/2],
                    #                                     [-time_ignore[obj_idx][12]/2,-time_ignore[obj_idx][12]/2,time_ignore[obj_idx][12]/2, time_ignore[obj_idx][12]/2],
                    #                                     [1,1,1,1]])
                    #     corners_for_this_object = rot_mat @ corners_at_origin
                    #     corners[obj_idx] = corners_for_this_object[0:-1, :].reshape([8,], order='F') # In x0y0x1y1x2y2x3y3 format
                    # raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(corners)

                    # Use detection center point to calculate similarity
                    # raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(time_ignore[:, 13:15]) # xy
                    raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(time_ignore[:, 10:16])   # hwlxyz

                    # raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(time_ignore[:, 6:10])
                else:
                    raw_data['gt_crowd_ignore_regions'][t] = np.empty((0, 6))   # hwlxyz
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.prematch_result:
                all_tracker_dets = raw_data['tracker_dets'][t]
                if np.any(gt_class_mask) & (len(raw_data['tracker_dets'][t]) > 0):
                    dist = euclidean_distances(all_tracker_dets[:,3:5], gt_dets[:,3:5])
                    tracker_class_mask = np.any(dist < self.prematch_distance_threshold, axis=1)
                else:
                    tracker_class_mask = np.zeros(len(raw_data['tracker_dets'][t])).astype(bool)
            else:
            ...
            # intersection_with_ignore_region = self._calculate_box_ious(unmatched_tracker_dets, crowd_ignore_regions, box_format='x0y0x1y1x2y2x3y3')
            intersection_with_ignore_region = self._calculate_euclidean_similarity(unmatched_tracker_dets, crowd_ignore_regions, zero_distance=2.0)
            ...
            to_remove_unmatched = unmatched_indices[is_within_crowd_ignore_region]
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        # similarity_scores = self._calculate_box_ious(gt_dets_t, tracker_dets_t, box_format='x0y0x1y1x2y2x3y3')
        ...
        similarity_scores = self._calculate_euclidean_similarity(gt_dets_t[:,3:5], tracker_dets_t[:,3:5], zero_distance=4.0)
