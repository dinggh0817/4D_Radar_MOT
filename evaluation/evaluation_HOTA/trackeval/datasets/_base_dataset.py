...
class _BaseDataset(ABC):
    ...
    #     used to determine if detections are within crowd ignore region.
    #     """
    #     if box_format != 'x0y0x1y1x2y2x3y3':
    #         if box_format == 'xywh':
    #             # layout: (x0, y0, w, h)
    ...
    #             return ious
    #     else:
    #         if bboxes1.size * bboxes2.size > 0:
    #             gious = generalized_box_iou(torch.tensor(bboxes1.astype(np.float32)), torch.tensor(bboxes2.astype(np.float32)))
    #             return np.array(gious).astype(np.float64)
    #         else:
