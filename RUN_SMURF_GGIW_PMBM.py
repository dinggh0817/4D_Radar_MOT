"""
Run the GGIW-PMBM filter to perform extended object tracking. Preprocess point clouds with SMURF detection results.
...
def parse_args():
    ...
    parser.add_argument("--results_dir",
        default=os.path.join(get_code_path(), "results/SMURF_GGIW_PMBM")
    )
...
def main(process_token, cls="default", output_dir=None, if_plot=False, cluster_by_bbox=True, if_print_frame_info=True):
    args = parse_args()
    ...
    for scene_idx, scene_token in enumerate(scenes_in_this_set):
        ...
        if args.detection == "GT":
            ...
            detection_bbox_in_this_scene = ground_truth_bbox_in_this_scene
            parameters[cls]['score_thld'] = 0
        else:
        ...
        for frame_idx, frame_token in enumerate(ordered_frames_in_this_scene):
            ...
            Z_k = gen_measurement_matrix_for_radar(
                point_cloud_at_current_frame, parameters[cls]['snr_thld'], parameters[cls]['vel_thld'], parameters[cls]['score_thld'], 3, cls,
                cluster_by_bbox, parameters[cls]['VOXEL_SIZE']
            )
            ...
            if frame_idx == 0:
                ...
                filter_predicted, meas_preprocessed = tracker.predict_initial_step(Z_k, cluster_by_bbox=cluster_by_bbox, ego_position=sensor_state)
            else:
                # Predict step
                filter_predicted, meas_preprocessed = tracker.predict(
                    time_lag, filter_pruned, Z_k, cluster_by_bbox=cluster_by_bbox, ego_position=sensor_state
                )
            te = perf_counter()
            tt = te - ts
...
def eval(out_file_dir, cls):
    ...
    eval_file_dir = os.path.join(output_dir, "evaluate", "SMRUF_GGIW_PMBM", "data")
...
if __name__ == "__main__":
    ...
    # Read config
    with open(os.path.join(args.tracker_dir, args.dataset + "_SMURF_config.json"), "rb") as f:
        config = ujson.load(f)
    ...
    for classification in classifications:
        ...
        for token in range(args.parallel_process):
            inputarguments.append(
                (token, classification, output_dir, False, True, False)
            )
        ...
        pool.close()
        # For debugging
        # main(0, classification, output_dir, True, True, False)
        print(">>> {} tracking fininshed.".format(classification))

    # For re-evaluating old results
